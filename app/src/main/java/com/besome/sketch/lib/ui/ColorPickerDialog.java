...
import org.sketchware.daygreen.CustomColorManager;
...
public class ColorPickerDialog extends PopupWindow {
	...
	public void initialize(Activity activity, String color, boolean isTransparentColor, boolean isNoneColor) {
		...
		CustomColorManager.migrateIfNecessary(colorPref);
	...
	private void showCustomColorCreatorDialog() {
		View customView = activity.getLayoutInflater().inflate(R.layout.dialog_color_picker_advanced, null);
		com.skydoves.colorpickerview.ColorPickerView pickerView = customView.findViewById(R.id.colorPickerView);
		com.skydoves.colorpickerview.sliders.AlphaSlideBar alphaBar = customView.findViewById(R.id.alphaSlideBar);
		com.skydoves.colorpickerview.sliders.BrightnessSlideBar brightnessBar = customView.findViewById(R.id.brightnessSlideBar);
		com.google.android.material.textfield.TextInputEditText hexEditText = customView.findViewById(R.id.hexEditText);
		com.google.android.material.card.MaterialCardView colorPreview = customView.findViewById(R.id.colorPreview);
		
		alphaBar.setSelectorDrawable(androidx.core.content.ContextCompat.getDrawable(activity, R.drawable.color_picker_selector));
		brightnessBar.setSelectorDrawable(androidx.core.content.ContextCompat.getDrawable(activity, R.drawable.color_picker_selector));
		...
		pickerView.setColorListener((com.skydoves.colorpickerview.listeners.ColorEnvelopeListener) (envelope, fromUser) -> {
			colorPreview.setCardBackgroundColor(envelope.getColor());
			if (fromUser) {
				isUpdatingFromPicker[0] = true;
				hexEditText.setText(envelope.getHexCode().toUpperCase());
				isUpdatingFromPicker[0] = false;
			}
		});
		...
		hexEditText.addTextChangedListener(new TextWatcher() {
			...
			public void onTextChanged(CharSequence s, int start, int before, int count) {
				if (!isUpdatingFromPicker[0] && (s.length() == 6 || s.length() == 8)) {
					try {
		...
		// Set initial hex text
		isUpdatingFromPicker[0] = true;
		hexEditText.setText(String.format("%08X", pickerView.getColor()).toUpperCase());
		isUpdatingFromPicker[0] = false;
		
		new MaterialAlertDialogBuilder(activity, R.style.ThemeOverlay_SketchwarePro_AlertDialog)
		...
		.setPositiveButton(Helper.getResString(R.string.common_word_save), (dialog, which) -> {
			...
			if (!hex.startsWith("#")) hex = "#" + hex;
			savePickedColor(hex.toUpperCase());
			notifyChanges();
		})
	...
	private void showCustomAttrCreatorDialog() {
		...
		dialog.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener(v -> {
			if (dialogBinding.inputLayout.getError() == null && !dialogBinding.input.getText().toString().isEmpty()) {
				...
				String savedAttrs = CustomColorManager.getCustomAttrs();
				String attrsToSave = savedAttrs + "," + attributeName;
				CustomColorManager.saveCustomAttrs(attrsToSave);
		...
		if (!savedColors.isEmpty()) {
			...
			while (index < colorStrings.length) {
				try {
					...
					colorBeans[index] = new ColorBean(colorStrings[index], "CUSTOM", count >= 2 ? "#212121" : "#ffffff", R.drawable.checked_white_32);
	...
	private void savePickedColor(String color) {
		...
		if (savedColors.contains(color)) {
			...
			CustomColorManager.saveCustomColors(colorsToSave);
			colorGroups.set(0, getSavedColorBeans());
			notifyChanges();
			k = 0;
			smoothScrollToCurrentItem();
	...
	public void initializeAttrsList() {
		...
		attributes.add(new Attribute("colorSurfaceDim"));
		attributes.add(new Attribute("colorSurfaceBright"));
		...
		attributes.add(new Attribute("colorPrimaryInverse"));
		...
		// Outline
		attributes.add(new Attribute("colorOutline", "Outline"));
		attributes.add(new Attribute("colorOutlineVariant"));
		
		// Container
		attributes.add(new Attribute("colorSurfaceContainerLowest", "Container"));
		attributes.add(new Attribute("colorSurfaceContainerLow"));
		attributes.add(new Attribute("colorSurfaceContainer"));
